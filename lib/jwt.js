...
function authFail(response){
  ...
  return response.end("you failed 401");
}


function authSuccess(request, response, accessToken){
  var token = generateAndStoreJWT(request, accessToken, response);
...
function generateAndStoreJWT(request, options, accessToken){
  var token = generateToken(request, accessToken, options);
  var record = {
    "valid": true,
    "created": new Date().getTime()
  };

  db.put(accessToken, JSON.stringify(record), function(err){
...
//generate the jwt
function generateToken(req, accessToken, options){
  options = options || {};
