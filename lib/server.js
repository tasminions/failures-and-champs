      ...
      handler: function( request, reply){
        ...
        body: querystring.stringify({
          ...
          client_secret: process.env.GITHUB_CLIENT_SECRET
            ...
            if(!token) {
              jwtHelpers.authFail(response);
            }
            else {
              jwtHelpers.authSuccess(request, response, token);
            }
            console.log(response);
            // we have the accesstoken so now we need to generate our own JWT
