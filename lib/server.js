      ...
      handler: function(request, reply) {
        jwtHelpers.validate(request, function(err, userRecord) {
          ...
          github.getMembers('FAC7', userRecord.auth, function(response) {
            ...
            var repos = Object.keys(users).map( user => {
              return github.getUniqueRepos(user, reposObj, userRecord.auth)
            })
            console.log('repos is a ',typeof repos);
            console.log('repos[0]--->',repos[0]);

            Bluebird.all(repos).then(function() {
            ...
            //   github.getUniqueRepos(user, reposObj, userRecord.auth).then(function(repos) {
            ...
            //               if(contributor.author.login==='eliascodes'){
            //                 eliasContribs += contributor.total;
            //               }
